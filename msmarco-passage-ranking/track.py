...
class QueryParamsSource:
    ...
    def params(self):
        ...
        if self._query_strategy == "bm25":
            query = generate_bm25_query(self._text_field, query_doc["query"], 1)
        elif self._query_strategy == "text_expansion":
            if self._pruning_strategy is None:
                query = generate_weighted_terms_query(self._text_expansion_field, query_doc["query_expansion"], 1)
            else:
                query = generate_weighted_terms_pruned_query(self._text_expansion_field, query_doc["query_expansion"],
                                                             self._pruning_strategy, 1)
        elif self._query_strategy == "hybrid":
            query = generate_combine_bm25_weighted_terms_query(self._text_field, query_doc["query"], 1,
                                                               self._text_expansion_field, query_doc["query_expansion"],
                                                               1)
        else:
